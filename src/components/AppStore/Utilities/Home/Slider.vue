<template>
  <div w="full" m-l="3" position="relative">
    ...
    <nuxt-img class="transition-all duration-300 ease-in delay-75"  relative="~" w="full" h="130" cursor="pointer" border="w-70" :src="banner.src" />
    ...
    <div position="absolute" bottom="-10" p="2" class="grid grid-flow-col overflow-x-scroll scroll-smooth snap-x" w="full" rounded="lg">
      <div v-for="app in apps" :key="app.id" m-r="2" cursor="pointer" grid="~ flow-row" h="40" w="40" :class="app.id == banner.app.id ? 'bg-w-30 transition ease-in-out delay-75 -translate-y-2 scale-x-110 border-success-300 snap-center' : 'bg-w-10 border-w-10' " rounded="lg" place="items-center" p="4" >
...
const { state:banner, next, prev } = useCycleList([
  {
