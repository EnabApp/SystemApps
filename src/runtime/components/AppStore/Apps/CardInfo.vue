<template>
  <div>
    ...
    <span text="primary 3xl">{{ breakpoint.size }}</span>
    <div
      :class="
    ...
    >
      <!-- Main Section -->
      <div
        transition="all duration-400 ease-in-out delay-75"
        :class="
          !(breakpoint.twoXs || breakpoint.xs || breakpoint.sm || breakpoint.md)
            ? 'w-60%'
            : 'w-95%'
      ...
      >
        ...
        <div flex="~ gap-3" items="center" w="full" h="auto" bg="info">
          ...
          <div h="full" bg="warning">
            ...
            <div flex="~">
              ...
              <!-- Owned -->
              <div
                v-else-if="appStore.selectedApp.owned"
                flex="~"
                justify="center"
                m-t="30px"
                w="122px"
              ...
              <div v-else flex="~">
                ...
                <span
                  flex="~"
                  text="md primaryOp dark:primary 2xl"
                  m-t="9"
                  m-r="3"
                >
                  <div class="i-ri:copper-coin-fill" w="32px" h="32px"></div>
                  {{
        ...
        <div m-t="6">
          ...
          <div v-if="appStore.selectedApp.getAllServices()">
            ...
            >
              <LazyAppStoreAppsServiceCard
                v-for="service in appStore.selectedApp.getAllServices()"
                :key="'service-' + service"
                :service="service"
              />
            </div>
          ...
          <div v-else>
            <span text="primaryOp dark:primary 2xl"
              >لا توجد خدمات لهذا التطبيق حاليا</span
            >
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <span text="priamryOp dark:primary" font="bold"
          >التطبيق متوفر في الحزم التالية :</span
        >
        <AppStoreAppsPackCard
          m-y="4"
          v-for="pack in packs"
          :key="'pack-' + pack.id"
          :pack="pack"
        />
        <span text="priamryOp dark:primary" font="light"
          >شراء الحزم يساعدك على توفير المال
        </span>
      </div>
    </div>
    <!-- Buy App Modal -->
